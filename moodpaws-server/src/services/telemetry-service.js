...
import { enrichEmotionWithAiAdvice } from './deepseek-service.js'
...
export async function getLatestEmotion() {
  ...
  const moodMetric =
    ...
    extractMoodFromMetricPoint(latestMoodRows.find((row) => row.metric_key === 'PetHouse:Mood')) ||
  ...
  const emotionPayload = {
    ...
    createdAt: Number(moodMetric?.ts ?? moodMetric?.time) || Date.now()
  }

  return enrichEmotionWithAiAdvice(emotionPayload)
}
